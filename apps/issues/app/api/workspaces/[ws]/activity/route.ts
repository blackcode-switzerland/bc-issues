...
//
// Class B (D-22): the feed itself is platform — `platform.events` holds every
// app's events — but an event's `entity_id` is an INTERNAL ROW ID, and for this
// app's own entities it must be swapped for the workspace #number before it
// leaves the server. That means reading `issues.*`, which a platform package
