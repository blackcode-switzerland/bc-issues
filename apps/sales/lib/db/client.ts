...
//
// This was not theoretical: the first `npm run build` of the scaffold this app
// was copied from failed with "Failed to collect page data for
// /api/workspaces/[ws]/notes" for exactly that reason. `apps/issues` does it at
// module scope and gets away with it only because a `.env.local` happens to be
// present.
//
// So: resolved on first use, memoised after. Keep this shape.
