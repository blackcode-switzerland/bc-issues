...
function buildLlmsTxt(): string {
  return [
    ...
    `- [Changelog](${m.discovery.changelog}): complete platform reference + dated changes (JSON at ${m.discovery.changelog_api}) — read this to get current`,
    `- [Get up to date](${m.discovery.get_up_to_date}): how an agent/skill brings itself current`,
    '',
    '## Staying current',
    `- ${m.staying_current}`,
    '',
