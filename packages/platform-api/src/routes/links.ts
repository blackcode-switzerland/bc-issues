// Typed relations between two URNs — `bk link create|list|rm`.
//
...
//
// `platform.links` is the only table in this area holding data that is NOT
...
//
// Cross-app by construction (D-18): a sales prospect linking to an issue is the
// point, so both ends are looked up in `platform.entities` and neither app's own
// tables are touched. Mounted by every app, unchanged.
...
import { createApiHandler, createResolveWorkspace, type WorkspaceContext } from '../handler'
...
export function linksRoute(app: AppContext) {
  ...
  const GET = apiHandler(async (req: NextRequest, { params }: Params) => {
    ...
    // to tell them apart.
    const [entity] = await getEntitiesByUrns(app.db, [urn])
    if (!entity) {
    ...
    return jsonList(await listLinks(app.db, urn))
  })
  ...
  const POST = apiHandler(async (req: NextRequest, { params }: Params) => {
    ...
    const result = await createLink(app.db, { fromUrn: from, toUrn: to, rel, createdBy: ctx.user.id })
    if (!result.ok) {
  ...
  const DELETE = apiHandler(async (req: NextRequest, { params }: Params) => {
    ...
    const removed = await deleteLink(app.db, { fromUrn: from, toUrn: to, rel })
    if (!removed) {
      throw Errors.notFound(
        'link_not_found',
