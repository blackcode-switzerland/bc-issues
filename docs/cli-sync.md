...
8. **Inline file embedding.** Three client paths, all relying on the server's `toRichTextHtml` (`upgradeUploadedMedia`) to rewrite uploaded-origin urls into TipTap nodes: (a) `--file` (on `issue/task/project create` and `issue comment`) uploads via `UploadFile` then appends `client.EmbedMarkdown(up)` — `![name](url)` for images, `[name](url)` otherwise; (b) `resolveBodyMedia` (`commands/media.go`) scans description/body/update/comment text for Markdown refs to **local files**, uploads each, and rewrites in place (angle-bracket form `(<path>)` required for paths with spaces/parens); (c) `bk upload` prints urls only. If the embed Markdown shape or the server's url-detection changes, keep `EmbedMarkdown` + `resolveBodyMedia` (Go) and `upgradeUploadedMedia` (`lib/rich-text.ts`) in sync. `--attach` (issue-only) is unchanged — it writes to the attachments table, not the body.
