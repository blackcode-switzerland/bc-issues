...
// An AGENTS.md written by a pre-3.0.0 bk carries the old markers. If they were
...
func TestUpsertAgentsSectionReplacesTheLegacyBlock(t *testing.T) {
	doc := "# My project\n\n" + legacyBlockBegin + "\nold body\n\n<!-- bk 3.0.0 -->\n" + legacyBlockEnd +
		"\n\n## After\n"
	got := UpsertAgentsSection(doc, RenderAgentsSection("3.0.0"))
...
func TestClassify(t *testing.T) {
	cases := []struct {
		...
		// THE MIGRATION GUARANTEE. Every skill file installed before 3.0.0
		...
		{"pre-3.0.0 output (old markers)", legacyBlockBegin + "\nbody\n\n" +
...
// --- the 3.0.0 rename: blackcode-issues -> blackcode (D-17) ------------------
...
func TestUpsertConvertsLegacyMarkersAndKeepsUserContent(t *testing.T) {
	...
	got, err := UpsertSkillFile(old, "3.0.0")
	if err != nil {
		t.Fatalf("UpsertSkillFile: %v", err)
	}
...
func TestPlanMigration(t *testing.T) {
	...
	t.Run("migrates a bk-owned file and keeps what the user added", func(t *testing.T) {
		plan, err := PlanMigration(managedOld, "", "3.0.0")
		if err != nil {
	...
	t.Run("does nothing once the new file exists", func(t *testing.T) {
		plan, err := PlanMigration(managedOld, Render("3.0.0"), "3.0.0")
		if err != nil {
			t.Fatalf("PlanMigration: %v", err)
		}
	...
	t.Run("refuses a hand-written old file, and removes nothing", func(t *testing.T) {
		plan, err := PlanMigration(handWritten, "", "3.0.0")
...
func TestMigrationRenamesTheFrontMatterIdentity(t *testing.T) {
	...
	plan, err := PlanMigration(old, "", "3.0.0")
	if err != nil {
		t.Fatalf("PlanMigration: %v", err)
	}
