			...
			return output.Render(format, tokens, func(w io.Writer) error {
				...
				for _, t := range tokens {
					...
					fmt.Fprintf(tw, "%d\t%s\t%s\t%s\t%s\t%s\t%s\n",
						...
						cmdutil.DerefOr(t.LastUsedAt, "never"),
						cmdutil.DerefOr(t.ExpiresAt, "never"),
						cmdutil.DerefOr(t.CreatedAt, ""))
				}
...
func newTokenDeleteCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !cmdutil.Confirm(fmt.Sprintf("Revoke token #%d? Any scripts using it will stop working.", id), yes) {
